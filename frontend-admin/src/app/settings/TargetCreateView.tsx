...
export default function TargetCreateView() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await fetchAPI("/api/alerts/targets", {
        method: "POST",
      ...
      setMessage({ type: "error", text: err.message || "Failed to create alert target" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="mobile-p-sm" style={{ padding: '40px 24px' }}>
      <div className="animate-fade-in mobile-gap-sm" style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div className="mobile-stack" style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
          ...
          <div>
            <h1 className="font-display mobile-text-lg" style={{ fontSize: '24px', fontWeight: '700', marginBottom: '4px' }}>New Alert Target</h1>
