...
func TestDashboardUpdateHandler_PreservesAllAgentsWhenNewAdded(t *testing.T) {
	...
	t.Run("correctly orders agents when agents_order is submitted as a JSON array string", func(t *testing.T) {
		mock := &mockWebStore{}
		handler := &WebHandler{
			Renderer:  renderer,
			Store:     mock,
		...
		form := url.Values{
			...
			"max_days":     {"7"},
			"agents":       {"agent-1", "agent-2", "agent-3"},
			"agents_order": {`["agent-2", "agent-3", "agent-1"]`}, // JSON array string in formvalue
			"is_dragged":   {"1"},
		}
		...
		expectedOrder := []string{"agent-2", "agent-3", "agent-1"}
		for i, exp := range expectedOrder {
			if mock.dashboardUpdatedAgents[i].AgentID != exp {
				t.Errorf("expected agent at index %d to be %s, got %s", i, exp, mock.dashboardUpdatedAgents[i].AgentID)
			}
